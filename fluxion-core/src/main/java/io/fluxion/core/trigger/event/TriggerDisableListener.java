...
package io.fluxion.core.trigger.event;
...
import io.fluxion.core.trigger.TriggerConfig;
import io.fluxion.core.trigger.cmd.TriggerDisableCmd;
    ...
    public void on(TriggerDisableCmd event) {
        ...
        switch (config.getTrigger().getType()) {
            case Trigger.Type.SCHEDULE:
                scheduledTaskEntityRepo.updateEnable(TriggerHelper.scheduleTaskId(triggerEntity.getTriggerId()), false);
                break;
        }
    }
}
