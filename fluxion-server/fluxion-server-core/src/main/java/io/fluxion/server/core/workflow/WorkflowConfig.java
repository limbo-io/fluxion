 ...
 */
@Data
public class WorkflowConfig implements ValidatableConfig {
    ...
    public List<ValidateSuppressInfo> validate() {
        ...
        if (nodes.isEmpty()) {
            infos.add(new ValidateSuppressInfo(WorkflowConstants.FLOW_NODE_EMPTY));
            return infos;
        }
        if (nodes.size() > WorkflowConstants.FLOW_NODE_MAX_SIZE) {
            infos.add(new ValidateSuppressInfo(WorkflowConstants.FLOW_NODE_OVER_LIMIT));
            return infos;
        }
        // 单个节点校验
        ...
        // dag校验
        DAG<WorkflowNode> dag = new DAG<>(nodes, edges);
        // 开始节点
        if (CollectionUtils.isEmpty(dag.origins())) {
            infos.add(new ValidateSuppressInfo(WorkflowConstants.FLOW_START_NODES_IS_EMPTY));
            return infos;
        }
        if (dag.origins().stream().anyMatch(n -> !Objects.equals(n.getType(), WorkflowNode.Type.START))) {
            infos.add(new ValidateSuppressInfo(WorkflowConstants.FLOW_START_NODES_HAS_ERROR_TYPE_NODE));
            return infos;
        }
        // 结束节点
        ...
        // 是否成环
        if (dag.hasCyclic()) {
            infos.add(new ValidateSuppressInfo(WorkflowConstants.FLOW_HAS_CYCLIC));
            return infos;
        }
        return infos;
    }
