...
package io.fluxion.platform.id.service;

import io.fluxion.common.utils.MD5Utils;
import io.fluxion.platform.dao.entity.IdEntity;
import io.fluxion.platform.dao.repository.IdEntityRepository;
...
public class IDService {
    ...
    public void registerIds() {
        for (IDType idType : IDType.values()) {
            ...
            IdEntity idEntity = idEntityRepository.findById(typeName).orElse(null);
            if (idEntity == null) {
    ...
    private ID getNewId(IDType type) {
        ...
        while (updateNum <= 0 && time < 10) {
            try {
                ...
                updateNum = idEntityRepository.casGainId(typeName, endId, startId);
                Thread.sleep(200);
