...
package io.fluxion.platform.schedule.scheduler;

import io.fluxion.platform.schedule.task.AbstractTask;
 ...
 */
@Slf4j
public class TaskScheduler<T extends AbstractTask> extends HashedWheelTimerScheduler<T> {
    ...
    public void schedule(T task) {
        ...
        try {
            ...
            calAndSchedule(task, buildConsumer());
        } catch (Exception e) {
    ...
    protected Consumer<T> buildConsumer() {
        return task -> {
            try {
                task.execute();
            } catch (Exception e) {
                log.error("[HashedWheelTimerScheduler] schedule fail id:{}", task.id(), e);
                stop(task.id());
            }
        };
    }
