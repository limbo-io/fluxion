...
import io.fluxion.platform.trigger.cmd.TriggerDisableCmd;
    ...
    public void on(TriggerDisableCmd event) {
        ...
        switch (config.getTrigger().getType()) {
            case Trigger.Type.SCHEDULE:
                scheduledTaskEntityRepository.updateEnable(TriggerHelper.scheduleTaskId(triggerEntity.getTriggerId()), false);
                break;
        }
    }
}
