...
package io.fluxion.platform.version.service;

import io.fluxion.common.utils.UUIDUtils;
import io.fluxion.platform.exception.ErrorCode;
import io.fluxion.platform.dao.entity.VersionEntity;
import io.fluxion.platform.dao.repository.VersionEntityRepository;
...
import io.fluxion.platform.version.model.VersionGenerateType;
...
public class VersionService {
    ...
    public VersionByIdQuery.Response handle(VersionByIdQuery query) {
        VersionEntity entity = versionEntityRepository.findById(new VersionEntity.ID(query.getRefId(), query.getRefType().type, query.getVersion())).orElse(null);
        if (entity == null) {
            return null;
        }
    ...
    public VersionCreateCmd.Response handle(VersionCreateCmd cmd) {
        ...
        versionEntityRepository.saveAndFlush(entity);
        return new VersionCreateCmd.Response(version);
    }

    @CommandHandler
    public VersionUpdateCmd.Response handle(VersionUpdateCmd cmd) {
        VersionEntity entity = versionEntityRepository.findById(new VersionEntity.ID(cmd.getRefId(), cmd.getRefType().type, cmd.getVersion())).orElse(null);
        if (entity == null) {
    ...
    private String incrVersion(String refId, String refType) {
        VersionEntity last = versionEntityRepository.findLast(refId, refType);
        long version = 1L;
        if (last != null) {
